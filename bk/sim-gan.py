...
def refiner_network(input_image_tensor):
    ...
    def resnet_block(input_features, nb_features=64, nb_kernel_size=3):
        ...
        """
        y = layers.Conv2D(filters=nb_features, kernel_size= nb_kernel_size,padding='same')(input_features)
       # y = layers.Convolution2D(nb_features, nb_kernel_rows, nb_kernel_cols, padding='same')(input_features)
        y = layers.Activation('relu')(y)
        y = layers.Conv2D(filters=nb_features, kernel_size= nb_kernel_size,padding='same')(y)
    ...
    x = layers.Conv2D(filters=64, kernel_size=3,padding='same', activation='relu')(input_image_tensor)
...
def discriminator_network(input_image_tensor):
    ...
    """
    x = layers.Conv2D(filters=96, kernel_size= 3, padding='same', strides=2, activation='relu')(input_image_tensor)
    x = layers.Conv2D(filters=64, kernel_size= 3, padding='same', strides=2, activation='relu')(x)
    x = layers.MaxPool2D(pool_size=(3, 3), padding='same', strides=(1, 1))(x)
    x = layers.Conv2D(filters=32,kernel_size=  3, padding='same', strides=1, activation='relu')(x)
    x = layers.Conv2D(filters=32, kernel_size= 1, padding='same', strides=1, activation='relu')(x)
    x = layers.Conv2D(filters=2,kernel_size= 1, padding='same', strides=1, activation='relu')(x)
...
if __name__ == '__main__':
    ...
    main('./dataset/gaze.h5', './dataset/real_gaze.h5', refiner_model_path, discriminator_model_path)
